...
export default function PlanCard({plan, isMonthlyPackage}) {
	...
	return (
		<div className='w-full md:w-1/2 lg:w-1/3 px-4 shrink fadeIn-hidden plan-card'>
			<div className='relative z-10 bg-primary bg-opacity-5 dark:bg-[#1D2144] px-8 py-10 rounded-lg select-none cursor-pointer hover:outline hover:outline-offset-0 hover:outline-primary transition-all h-full'>
				<h3 className='mb-2 text-4xl text-center font-semibold leading-tight text-black dark:text-white'>
					{plan_name}
				</h3>
				<h4 className='font-bold text-black text-center dark:text-white text-xl'>{isMonthlyPackage ? price.m : price.y}</h4>
				<p className='text-base text-body-color mb-7 mt-3'>
					Lorem ipsum dolor sit amet adiscing elit Mauris egestas enim.
				</p>
				<div className='border-b border-body-color dark:border-white border-opacity-10 dark:border-opacity-10 pb-8 mb-8 cursor-pointer select-none'>
					<span className='font-semibold text-base text-white bg-primary w-full flex items-center justify-center rounded-md p-3 hover:bg-opacity-80 transition duration-300 ease-in-out'>
						Start Free Trial
					</span>
				</div>
				<div className='list-services'>
					{services.map((s, i) => (
						<div className='flex items-center mb-3' key={i}>
							...
							<p className='text-base font-medium text-body-color m-0'>{s.service_name}</p>
						</div>
					))}
				</div>
